...
현재 저장소는 Phase 4 MCP 호출 로그 기반 구현 단계입니다. 앱 골격과 더미 데이터는 준비되어 있고, FastAPI backend에서 SQLite seed, 인증, A기업 메뉴 조회, 주문 생성, Mock 결제, 포인트, 영수증, 관리자 주문 API, Burger MCP tool 호출 로그를 제공합니다.
...
현재 frontend 화면들은 실행 확인용 scaffold입니다. 챗봇 로직과 실제 화면 연결은 다음 Phase에서 추가합니다.
